...
public function fetchProductList($cat_id, $page = 1, $page_size = 10, $filters = array()) {
    ...
    public function fetchPendingForProcessing($limit = 20) {
        ...
        // Re-queue stuck processing rows so they can be imported first.
        // Use fetched_at as a conservative age check (older installs may not have processing timestamp).
        try {
            $this->db->query(
                "UPDATE `" . $tbl . "`
                 SET `status` = 'pending'
