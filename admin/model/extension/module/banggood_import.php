...
protected function apiRequestRawSimple($url) {
   ...
   protected function updateExistingProduct($product_id, $normalized) {
    ...
    /* -------------------------
       ...
       - Generates the same /image/cache/... filenames OpenCart uses
       - Performs resize/encode via Imagick to avoid GD-WebP crashes
       ------------------------- */
    protected function generateImageCacheForImages(array $images) {
        ...
        // Determine which extension OpenCart uses for cached images.
        // Many WebP mods store it as config_image_extension or config_image_default_extension.
        ...
        foreach ($images as $img) {
            ...
            // OpenCart cache naming scheme:
            // cache/<relative_path_without_ext>-<w>x<h>.<ext>
            $baseNoExt = $img_rel;
            ...
            foreach ($sizes as $s) {
                ...
                try {
                    ...
                    // Honor EXIF orientation when supported
                    if (method_exists($im, 'autoOrient')) {
                        try { $im->autoOrient(); } catch (\Throwable $e) {}
                    }

                    // Resize while preserving aspect ratio, then letterbox to exact WxH like OpenCart does.
                    $im->setImageBackgroundColor(new \ImagickPixel('transparent'));
                    $im->thumbnailImage($w, $h, true, true);
                    $im->extentImage($w, $h, (int)(($im->getImageWidth() - $w) / 2), (int)(($im->getImageHeight() - $h) / 2));

                    // Format + quality
                    $im->setImageFormat($outExt);
                    if (in_array($outExt, array('jpg', 'jpeg', 'webp'), true)) {
                        $im->setImageCompressionQuality(90);
                        if ($outExt === 'webp') {
                            try { $im->setOption('webp:method', '6'); } catch (\Throwable $e) {}
                        }
                    }
                    ...
                    $im->writeImage($cacheFull);
                    $im->clear();
                    $im->destroy();
                } catch (Exception $e) {
                    // Best effort: never fail import for cache warm errors
                }
            }
        }
    }
